...
get_thread_by_pid(pid_t pid){
    ...
    lock_acquire(lock_PIDTable);
    target_thread = array_getguy(PID_table, pid);
    lock_release(lock_PIDTable);
...
{ 
    ...
    thread->t_cwd = NULL;    
    thread->parent = NULL;
    thread->has_exited = false; 
    thread->lock_wait_for_child = lock_create("waiting for child to exit");
    thread->cv_wait_for_child = cv_create("sleep on child's address");
    thread->child_has_exited = sem_create("making sure parent doesnt exit before child",1);
    thread->parent_has_waited = lock_create("making sure parent has waited before destroying child");
    thread->parent_waited = false; 
    
    /* Modification to PID_table needs to be atomic */
    lock_acquire(lock_PIDTable); 
    if(num_PID == 25){
        for(index=0; index<25; index++){
            temp=array_getguy(PID_table, index);
            if(temp == NULL){
                thread->t_pid = index; 
                array_setguy(PID_table, index, thread);
                break; 
            }
        }
    }
    else {
    thread->t_pid = num_PID;
    array_setguy(PID_table, num_PID, thread);
    num_PID++;
    }
    lock_release(lock_PIDTable);
    splx(spl);
...
{   
    ...
    assert(numthreads>0);
    numthreads--;
    curthread->has_exited = true; 
    /* signal parent that child has exited */
    lock_acquire(curthread->lock_wait_for_child);
    cv_signal(curthread->cv_wait_for_child, curthread->lock_wait_for_child);
    lock_release(curthread->lock_wait_for_child);
    V(curthread->parent->child_has_exited);
    mi_switch(S_HALFZOMB);
...
{   
    ...
    kprintf("thread %d is being destroyed\n", thread->t_pid);
    ...
    //kfree(thread->parent);
    lock_destroy(thread->lock_wait_for_child);
    cv_destroy(thread->cv_wait_for_child);
    sem_destroy(thread->child_has_exited);
    lock_destroy(thread->parent_has_waited);
...
static
void
before_exorcise(void)
{
    int index, num_halfzombies;
    struct thread *child, *parent, *temp;
    num_halfzombies = array_getnum(half_zombies);
    for (index = 0; index < num_halfzombies; index++){
        child = array_getguy(half_zombies, index);
        if (child != NULL){ //if founcd child
                if(child->parent_waited == true){
                    if(child->has_exited == true){
                        array_setguy(PID_table, index, NULL);
                    }
                    kprintf("adding into zombies\n");
                array_add(zombies, child);
                array_setguy(half_zombies, index, NULL);
        }
    }
}
exorcise();
...
exorcise(void)
{
    ...
    for (i=0; i<array_getnum(zombies); i++) {
        ...
        assert(z!=curthread);
        kprintf("thread being destroyed is %d\n", z->t_pid);
        thread_destroy(z);
    }
...
thread_bootstrap(void)
{
    ...
    PID_table = array_create();
    if (PID_table==NULL) {
        panic("Cannot create PID_table array\n");
    }
    result = array_preallocate(PID_table, MAXPID+1);
     if (result) {
        panic("Cannot preallocate PID table array\n");
    }
    result = array_setsize(PID_table, MAXPID);
    if (result) {
        panic("Cannot set PID table array size\n");
    }
    half_zombies = array_create();
     if (half_zombies==NULL) {
        panic("Cannot create half zombies array\n");
    }
    result = array_preallocate(half_zombies, MAXPID+1);
     if (result) {
        panic("Cannot preallocate half zombies array\n");
    }
    result = array_setsize(half_zombies, MAXPID);
    if (result) {
        panic("Cannot set half zombies size\n");
    }

    /* lock for PID table */
    lock_PIDTable = lock_create("lock for PID table");
     ...
     */
    execv_lk = lock_create("Exec_lock");
        ...
        struct thread **ret)
{
    ...
    newguy->ppid = curthread->t_pid;
    newguy->parent = curthread;
    /* Child does not have to worry about this sem */
    P(curthread->child_has_exited);
    s = spl0(); 
    /* Allocate a stack */
...
mi_switch(threadstate_t nextstate)
{
    ...
    else if (nextstate==S_HALFZOMB) {
        //kprintf("soon-to-be zombie is %s\n", cur->t_name);
        array_setguy(half_zombies, cur->t_pid, cur);
        result = 0; 
    }
    else {
