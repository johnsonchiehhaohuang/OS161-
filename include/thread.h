...
/* Array for storing all processes with PID indices */
struct array;
extern struct array *PID_table;
struct lock;
extern struct lock *lock_PIDTable;
struct addrspace;
...
struct thread {
    ...
    struct lock* lock_wait_for_child; 
    struct cv* cv_wait_for_child; 
    int has_exited; 
    struct semaphore* child_has_exited; 
    struct lock* parent_has_waited; 
    int parent_waited; 
};

extern struct lock* execv_lk;
...
/* get the desired thread from the process array through its PID */
...
/*
 * Wake one thread up only
 * Implemented for CV_signal function
 */
