...
{
	//kprintf("calling cmd_progthread\n");
	char **args = ptr;
	char progname[128];
	int result;

	assert(nargs >= 1);
	//kprintf("number of arguemnts is %d\n", nargs);
	//kprintf("arguments are %s, %s, %s\n", args[0],args[1],args[2]);
	// if (nargs > 2) {
...
common_prog(int nargs, char **args)
{
	...
	result = thread_fork(args[0] /* thread name */,
			...
			cmd_progthread, &waiting_on);
	if (result) {
		panic("thread_fork failed: %s\n", strerror(result));
		return result;
	} 
	result =  sys_waitpid(waiting_on->t_pid, &status,0, &retval);
	if (result) {
		panic("sys_waitpid failed: %s\n", strerror(result));
		return result;
	} 
